...
function checkCashRegister(price, cash, cid) {
  let owed = cash * 100 - price * 100;
  let values = [1, 5, 10, 25, 100, 500, 1000, 2000, 10000]
  let given;
  let tillEmpty = false;

  let change = cid.reduceRight((acc, val, i) => {
    if(owed % values[i] < owed){
      given = val[1] * 100 < owed ? val[1] * 100 : Math.floor(owed / values[i]) * values[i]
      tillEmpty = val[1] * 100 == given ? true : false;
      owed -= given
      return acc.concat([[val[0], given / 100]])
    }
    return acc
  }, []);
  return tillEmpty ? owed > 0 ? "Insufficient Funds" : "Closed" : change
}
